...
In pixel-based image analysis, each pixel is classified independently based on spectral signature. This method includes sub-pixel analysis, which estimates the fractional coverage of different land cover types within a single pixel [@castillejo2009].
...
-   **Common Uses** Widely used in agricultural monitoring, where detailed differentiation between crop types within a single pixel is critical. This is illustrated below [@castillejo2009]

```{r echo=FALSE, out.width="70%", fig.cap="Superpixel Analysis - [Mclachlan et al., 2017](https://www.tandfonline.com/doi/pdf/10.1080/01431161.2017.1346403?needAccess=true&)"}
knitr::include_graphics("img/subpixelanalysis.png")
```

The second method is superpixel analysis. This method groups similar pixels into larger *superpixels* for more efficient processing and analysis [@jensen2008] .
...
-   **Weaknesses** The choice of parameters for clustering can greatly affect the outcome, and inappropriate settings may lead to over- or under-segmentation [@jensen2008]
-   **Common Uses** Useful for large-scale land cover mapping where you are delineating homogeneous areas. K-means clustering are aid in creating super-pixels[@castillejo2009]

```{r echo=FALSE, out.width="70%", fig.cap="Subpixel Analysis - [Liu et al., 2022](https://www.mdpi.com/2072-4292/14/4/899)"}
knitr::include_graphics("img/superpixelanalysis.webp")
```

### Object-Based Image Analysis (OBIA)

OBIA groups adjacent pixels into *objects* based on their spectral characteristics. This approach considers the texture of features within an image [@jensen2008].

-   **Strengths:** Good at handling spatial patterns and relationships between objects, reducing 'salt and pepper' noise
-   **Weaknesses:** computationally intensive and requires parameter tuning for segmenting images into meaningful objects [@gorelick2017]
...
-   Confusion Matrix (Error Matrix): A powerful tool that cross-tabulates the actual categories (from ground truth) against the classified categories. It provides metrics like overall accuracy, user’s and producer’s accuracy, and Kappa coefficient [@jensen2008].

-   Producer’s and User’s Accuracy: These metrics derive from the confusion matrix. Producer's accuracy measures the probability that a ground-truth pixel is correctly classified, while user's accuracy shows the probability that a pixel classified into a category actually represents that category on the ground [@jensen2008].
...
-   Spatial and Temporal Mismatches: Differences in the timing of satellite imagery and the collection of reference data can introduce errors due to changes in the landscape or land use [@gorelick2017].
...
knitr::include_graphics("img/biodiversity.jpeg")
```
